...
export function RestaurantView() {
  const { requests, addRequest, awardBid, getBidsForRequest, purveyors, getOfferingsForPurveyor } = useMarketplace()
  const [tab, setTab] = useState<'requests' | 'purveyors'>('requests')
  ...
  return (
    <div>
      {/* Tab switcher */}
      <div className="flex items-center gap-2 mb-6 bg-white/5 border border-white/8 rounded-xl p-1 w-fit">
        <button
          onClick={() => setTab('requests')}
          className={`px-3.5 py-1.5 rounded-lg text-[13px] font-medium transition-all ${
            tab === 'requests' ? 'bg-[rgb(var(--surface-container-high))] text-foreground shadow-sm' : 'text-muted hover:text-foreground'
          }`}
        >
          📋 My Requests
        </button>
        <button
          onClick={() => setTab('purveyors')}
          className={`px-3.5 py-1.5 rounded-lg text-[13px] font-medium transition-all ${
            tab === 'purveyors' ? 'bg-[rgb(var(--surface-container-high))] text-foreground shadow-sm' : 'text-muted hover:text-foreground'
          }`}
        >
          🛒 Browse Purveyors
        </button>
